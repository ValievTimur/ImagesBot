...
users = []
request = {}
mes = {'helping': '''Отправь мне запрос, а я найду по нему картинку и отправлю тебе. Также я могу отправить тебе текстовое описание отправленной тобой картинки.\n\nНекоторые запросы на русском языке могут плохо обрабатываться, поэтому лучше использовать английский. \n\nДля работы бота просто отправь карттинку или текстовый запрос.''', 
'start': 'Отправь мне запрос, а я найду по нему картинку и отправлю тебе. Также я могу описать изображение!', 
...
def get_des(photo):
    ...
    values = []
    for i in data:
        values += i['label'].split(', ')
    items = ''
    for i in values:
        answer = t.translate(i, targetlang="ru")
        items += f'{answer.text.lower()}, '
    items = items.strip(', ')
